...
### 多源搜索策略（默认启用）
当用户请求搜索信息时，**默认同时使用多个搜索源**以获取更全面的信息：

| 搜索源 | 工具 | 适用场景 |
|--------|------|----------|
| **Web 搜索** | WebSearch / Brave Search | 通用信息、新闻、技术文档 |
| **小红书** | xiaohongshu-mcp | 生活经验、产品评测、用户口碑 |
| **学术文献** | zotero MCP | 论文、研究、学术观点 |

**执行顺序**（串行，避免速率限制）：
1. WebSearch - 获取通用网页结果
2. 小红书搜索 - 获取用户经验和评价（如果话题相关）
3. Zotero 语义搜索 - 获取学术观点（如果话题相关）
